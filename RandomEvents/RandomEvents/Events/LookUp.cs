...
namespace Bannerlord.RandomEvents.Events
{
    public sealed class LookUp : BaseEvent
    {
        ...
        {
            if (!eventDisabled)
                if (treeShakeChance != 0 || baseRangeChance != 0 || minRangeLevel != 0 || maxRangeLevel != 0 ||
                    minGold != 0 || maxGold != 0)
                    return true;
        ...
        public override void StartEvent()
        {
            ...
            var eventDescription =
                new TextObject(
                        "{=LookUp_Event_Desc}While walking past some trees you notice something shiny high up in its branches.")
                    .ToString();
                ...
                elements =>
                {
                    switch ((string)elements[0].Identifier)
                    {
                        case "a" when MBRandom.RandomFloat <= treeShakeChance:
                        {
                            ...
                            var eventOutcome1 =
                                new TextObject(
                                        "{=LookUp_Event_Text_1}You eventually shake the shiny object free from the tree! It hits the ground with a heavy thunk. It turns out that it was a purse with {goldGained} gold inside.")
                                    .SetTextVariable("goldGained", goldGained)
                                    .ToString();

                            InformationManager.ShowInquiry(
                                new InquiryData(eventTitle, eventOutcome1, true, false, eventButtonText2, null, null,
                                    null), true);
                            break;
                        }
                        case "a":
                            ...
                            var eventOutcome2 =
                                new TextObject(
                                        "{=LookUp_Event_Text_2}Try as you might, you're unable to get dislodge the shiny object.")
                                    .ToString();

                            InformationManager.ShowInquiry(
                                new InquiryData(eventTitle, eventOutcome2, true, false, eventButtonText2, null, null,
                                    null), true);
                            break;
                        case "b":
                            var eventOutcome3 =
                                new TextObject(
                                        "{=LookUp_Event_Text_3}You decide to leave the tree alone. Throughout the next few hours you can't help but wonder it was...")
                                    .ToString();
                            InformationManager.ShowInquiry(
                                new InquiryData(eventTitle, eventOutcome3, true, false, eventButtonText2, null, null,
                                    null), true);
                            break;
                        case "c":
                        {
                            //Get weapon
                            var skillToUse = GetSkillObject();

                            if (skillToUse == null)
                            {
                                var eventOutcome4 =
                                    new TextObject(
                                            "{=LookUp_Event_Text_4}Something went wrong with selecting your weapon, what have you done?! Aborting event.")
                                        .ToString();

                                InformationManager.ShowInquiry(
                                    new InquiryData(eventTitle, eventOutcome4, true, false, eventButtonText3, null,
                                        null, null), true);
                                return;
                            }
                            ...
                            else
                            {
                                ...
                                chancePercent = ((float)heroSkillValue - minRangeLevel) /
                                                (maxRangeLevel - minRangeLevel);
                            ...
                            if (MBRandom.RandomFloat < chancePercent)
                            {
                                ...
                                Hero.MainHero.AddSkillXp(skillToUse,
                                    GeneralSettings.Basic.GetLevelXpMultiplier() *
                                    Hero.MainHero.GetSkillValue(skillToUse));


                                var eventOutcome5 = new TextObject(
                                        "{=LookUp_Event_Text_5}You manage to knock the shiny object out of the tree with (what you consider) a fantastic shot! Shame no one was there to see it. You notice that object was in fact a purse full of {goldGained} gold!")
                                    .SetTextVariable("goldGained", goldGained)
                                    .ToString();

                                InformationManager.ShowInquiry(
                                    new InquiryData(eventTitle, eventOutcome5, true, false, eventButtonText2, null,
                                        null, null), true);
                            }
                            else
                            {
                                ...
                                var eventOutcome6 = new TextObject(
                                        "{=LookUp_Event_Text_6}Shot after shot you attempt to knock down the object without success. At one stage a bird drops out of the tree. It's time to give up... At least you have dinner.")
                                    .ToString();

                                InformationManager.ShowInquiry(
                                    new InquiryData(eventTitle, eventOutcome6, true, false, eventButtonText2, null,
                                        null, null), true);
                            }
        ...
        private static SkillObject GetSkillObject()
        {
                 ...
                 equipmentIndex++)
            {
                ...
                if (item != null && item.Type == ItemObject.ItemTypeEnum.Thrown) return DefaultSkills.Throwing;

                if (item != null && item.Type == ItemObject.ItemTypeEnum.Bow) return DefaultSkills.Bow;
                if (item != null && item.Type == ItemObject.ItemTypeEnum.Crossbow) return DefaultSkills.Crossbow;
            }
