    ...
    public sealed class DiseasedCity : BaseEvent
    {
        ...
        public override void StartEvent()
        {
            try
            {
                ...
                var useSkill = highestMedicineHero != null &&
                               highestMedicineHero.GetSkillValue(DefaultSkills.Medicine) >= highMedicineLevel;
        ...
        {
            if (useSkill) return MBRandom.RandomFloatRanged(0, 1) >= highMedicineChance;
        ...
        private void ResolvePlague(bool plagueKills, Settlement plaguedSettlement, Hero highestMedicineHero)
        {
            ...
            var eventOutcome1 =
                new TextObject(
                        "{=DiseasedCity_Event_Choice_1}{plaguedSettlement} has suffered a devastating plague. Although {highestHero} tried their best to save the population, it wasn't enough...")
            ...
            var eventOutcome2 = new TextObject(
                    "{=DiseasedCity_Event_Choice_2}{plaguedSettlement} has suffered a devastating plague. As there wasn't anyone able to provide assistance to the population, the sickness cut through the population without mercy.")
                .SetTextVariable("plaguedSettlement", plaguedSettlement.Name)
                .ToString();

            var eventOutcome3 = new TextObject(
                    "{=DiseasedCity_Event_Choice_3}Although the telltale signs of an emerging plague started to appear in {plaguedSettlement}, because of {highestHero}'s expertise, measures were put in place that saved the settlement from unnecessary death.")
                .SetTextVariable("plaguedSettlement", plaguedSettlement.Name)
                .SetTextVariable("highestHero", highestMedicine?.Name)
                .ToString();

            var eventOutcome4 = new TextObject(
                    "{=DiseasedCity_Event_Choice_4}Although the telltale signs of an emerging plague starting to appear in {plaguedSettlement}, as luck would have it, nothing ever came of it. Those that were ill recovered, and the fears of a deadly pandemic can be laid to rest... For now.")
                .SetTextVariable("plaguedSettlement", plaguedSettlement.Name)
                .ToString();
            ...
            if (plagueKills)
            {
                if (highestMedicineHero != null)
                {
                    ...
                    plaguedSettlement.Town.GarrisonParty?.MemberRoster.RemoveNumberOfNonHeroTroopsRandomly(
                        (int)(plaguedSettlement.Town.GarrisonParty.MemberRoster.TotalManCount * percentLoss));

                    // Kill some of their Militia
                    plaguedSettlement.MilitiaPartyComponent.Party.MemberRoster.RemoveNumberOfNonHeroTroopsRandomly(
                        (int)(plaguedSettlement.MilitiaPartyComponent.Party.MemberRoster.TotalManCount * percentLoss));

                    // Drop some loyalty
                    plaguedSettlement.Town.Loyalty *= 1 - percentLoss;

                    // Give the hero half xp for trying
                    var xpToGive = GeneralSettings.Basic.GetLevelXpMultiplier() *
                                   highestMedicineHero.GetSkillValue(DefaultSkills.Medicine) * 0.5f;

                    highestMedicineHero.AddSkillXp(DefaultSkills.Medicine, xpToGive);

                    InformationManager.ShowInquiry(
                        new InquiryData(eventTitle, eventOutcome1, true, false, eventButtonText, null, null, null),
                        true);
                }
                else
                {
                    ...
                    InformationManager.ShowInquiry(
                        new InquiryData(eventTitle, eventOutcome2, true, false, eventButtonText, null, null, null),
                        true);
                }
            }
            else
            {
                if (highestMedicineHero != null)
                {
                    ...
                    var xpToGive = GeneralSettings.Basic.GetLevelXpMultiplier() *
                                   highestMedicineHero.GetSkillValue(DefaultSkills.Medicine);

                    highestMedicineHero.AddSkillXp(DefaultSkills.Medicine, xpToGive);

                    // Thanks to hero's efforts lives were saved
                    InformationManager.ShowInquiry(
                        new InquiryData(eventTitle, eventOutcome3, true, false, eventButtonText, null, null, null),
                        true);
                }
                else
                {
                    ...
                    InformationManager.ShowInquiry(
                        new InquiryData(eventTitle, eventOutcome4, true, false, eventButtonText, null, null, null),
                        true);
                }
            }
        }
