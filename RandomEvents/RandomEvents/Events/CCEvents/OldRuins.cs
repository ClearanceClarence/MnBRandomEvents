    ...
    public sealed class OldRuins : BaseEvent
    {
        ...
        public override bool CanExecuteEvent()
        {
            return HasValidEventData() && MobileParty.MainParty.CurrentSettlement == null &&
                   MobileParty.MainParty.MemberRoster.TotalRegulars >= maxMen;
        }
        ...
        public override void StartEvent()
        {
            ...
            var eventOption1Hover =
                new TextObject("{=OldRuins_Event_Option_1_Hover}Better to check the homestead").ToString();
            ...
            var eventOption3Hover =
                new TextObject("{=OldRuins_Event_Option_3_Hover}Barns always hold something interesting").ToString();

            var eventOption4 = new TextObject("{=OldRuins_Event_Option_4}The small shack").ToString();
            var eventOption4Hover =
                new TextObject("{=OldRuins_Event_Option_4_Hover}The shack might be interesting").ToString();

            var eventOption5 = new TextObject("{=OldRuins_Event_Option_5}None of them, just leave").ToString();
            var eventOption5Hover =
                new TextObject("{=OldRuins_Event_Option_5_Hover}You don't want to get wet").ToString();
                ...
                elements =>
                {
                    switch ((string)elements[0].Identifier)
                    {
                        case "a":
                            ...
                            MobileParty.MainParty.MemberRoster.RemoveNumberOfNonHeroTroopsRandomly(killedMen);
                            break;
