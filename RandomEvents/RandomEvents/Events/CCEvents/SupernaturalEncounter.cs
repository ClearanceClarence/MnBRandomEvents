...
namespace Bannerlord.RandomEvents.Events.CCEvents
{
    public sealed class SupernaturalEncounter : BaseEvent
    {
        ...
        {
            return HasValidEventData() && !GeneralSettings.SupernaturalEvents.IsDisabled() &&
                   MobileParty.MainParty.CurrentSettlement == null && CurrentTimeOfDay.IsNight &&
                   MobileParty.MainParty.MemberRoster.TotalRegulars >= 50;
        }
        ...
        public override void StartEvent()
        {
            ...
            var eventOption1 =
                new TextObject("{=SupernaturalEncounter_Event_Option_1}Follow the apparition").ToString();
            var eventOption1Hover =
                new TextObject("{=SupernaturalEncounter_Event_Option_1_Hover}What could she possibly want?").ToString();

            var eventOption2 = new TextObject("{=SupernaturalEncounter_Event_Option_2}Try talking to her").ToString();
            var eventOption2Hover =
                new TextObject("{=SupernaturalEncounter_Event_Option_2_Hover}Seems like the logical choice").ToString();

            var eventOption3 = new TextObject("{=SupernaturalEncounter_Event_Option_3}Scream and run away").ToString();
            var eventOption3Hover =
                new TextObject("{=SupernaturalEncounter_Event_Option_3_Hover}I swear I'm not crazy!").ToString();

            var eventOption4 = new TextObject("{=SupernaturalEncounter_Event_Option_4}Ignore it").ToString();
            var eventOption4Hover =
                new TextObject("{=SupernaturalEncounter_Event_Option_4_Hover}It's just a dream anyway").ToString();
